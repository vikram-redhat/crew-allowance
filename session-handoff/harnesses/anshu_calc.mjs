...
const buf = await readFile(path.join(ASSETS, "Anshu-Jan-EOM-Schedule.pdf"));
...
console.error(`>> parsed.sectors.length = ${parsed.sectors.length}`);
console.error(`>> first sector = ${JSON.stringify(parsed.sectors[0])}`);
...
const svBuf = await readFile(path.join(ASSETS, "Sector Value_Jan'26.xlsx"));
...
const res = runCalculations("January 2026", sectors, scheduledTimes, svFiltered, PILOT, null, parsed.hotels || []);
